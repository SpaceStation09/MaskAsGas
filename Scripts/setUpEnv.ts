...
import { setUpMask, setUpUniswap } from "../test/setUp";
...
async function main() {
  ...
  normalProvider = new providers.Web3Provider(web3provider);
  ...
  await setUpPaymaster(relayHubAddress, forwarderAddress);
...
async function setUpPaymaster(relayHubAddress: string, forwarderAddress: string) {
